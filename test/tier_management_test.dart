  ...
  group('Subscription Tier Management Tests', () {
    ...
    group('Tier Pricing', () {
      test('Pricing increases with tier level', () {
        ...
        expect(
            SubscriptionTier.starter.monthlyPrice <
                SubscriptionTier.professional.monthlyPrice,
            true);
        expect(
            SubscriptionTier.professional.monthlyPrice <
      ...
      test('Discount percentage is consistent', () {
        ...
        final starterDiscount =
            ((starter.monthlyPrice * 12 - starter.yearlyPrice!) /
                    (starter.monthlyPrice * 12)) *
                100;
        final proDiscount = ((pro.monthlyPrice * 12 - pro.yearlyPrice!) /
                (pro.monthlyPrice * 12)) *
            100;
    ...
    group('Tier Limits', () {
      ...
      test('Limits increase with tier level', () {
        ...
        expect(free.maxEmployees < starter.maxEmployees, true);
        expect(
            starter.maxEmployees < pro.maxEmployees || pro.maxEmployees == -1,
            true);
      });
      ...
      test('Unlimited is represented as -1', () {
        ...
        expect(
            enterprise.maxEmployees, -1); // Enterprise has unlimited employees
        expect(enterprise.maxWorkplaces,
            -1); // Enterprise has unlimited workplaces
    ...
    group('Tier Features', () {
      ...
      test('Enterprise tier includes advanced features', () {
        ...
        final featureList =
            enterprise.features.map((f) => f.toLowerCase()).toList();
        expect(featureList.any((f) => f.contains('unlimited')), true);
        expect(
            featureList
                .any((f) => f.contains('api') || f.contains('integration')),
            true);
      });
    });
